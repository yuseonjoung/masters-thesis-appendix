---
title: "Regression_Analysis_Other_Consumption"
output:
...
# Dependent variables: CO2_other_consumption


df1 <- as_tibble(df1_1)
head(df1)

df1 <- df1 %>% select(2, 3, 4, 5, 6, 7, 22) #10, 20, 21, 22, 24
...
# Independent variables: age, income, political_party, education, urban_rural, federal_state
# Dependent variables: belief_diff_other_consumption
...
df2 <- df2 %>% select(2, 3, 4, 5, 6, 7, 28) #25, 26, 27, 28, 29
...
### II. Multivariate Regression: CO2 other consumption 
...
```{r}
# finding the most frequent values for the categorical variables 
...
model1 <- lm(CO2_other_consumption ~ age + income + political_party + education +  urban_rural_class + federal_state, data = df1)
...
```{r}
# Calculating the threshold for multicollinearity 
...
```{r}
### Backward regression using AIC: starting with all of the variables - best
...
cook = cooks.distance(step_model1)
plot(cook,
     type="h",
     lwd=3,
     ylab = "Cook's Distance",
...
influential = cooks.distance(step_model1)[which(cook >1)]
...
plot(df1$education, res1, xlab = "Income", ylab = "Residuals")
abline(h = 0)
...
#Non-linear transformation to the selected variables 

options(scipen = -2)
model_extra <- lm(sqrt(CO2_other_consumption) ~ age + income + education, data = df1)
...
res1 = stdres(model_extra) ## (Standardized) Residuals
...
plot(fitted(model_extra), res1, xlab = "Fitted values", ylab = "Residuals")
abline(h = 0)
```
...
### III. Multivariate Regression: belief diff other consumption 
...
model2 <- lm(belief_diff_other_consumption ~ age + income + political_party + education +  urban_rural_class + federal_state, data = df2)
summary(model2) 
...
cook = cooks.distance(step_model2)
plot(cook,
     type="h",
     lwd=3,
     ylab = "Cook's Distance",
