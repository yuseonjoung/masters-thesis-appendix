---
title: "Regression_Analysis_Mobility"
...
# Dependent variables: CO2_mobility

df1 <- as_tibble(df1_1)
head(df1)

df1 <- df1 %>% select(2, 3, 4, 5, 6, 7, 20) #10, 20, 21, 22, 24
...
```{r}
# Independent variables: age, income, political_party, education, urban_rural, federal_state
# Dependent variables: belief_diff_mobility
...
df2 <- df2 %>% select(2, 3, 4, 5, 6, 7, 26) #25, 26, 27, 28, 29
...
### II. Multivariate Regression: CO2 mobility
...
model1 <- lm(CO2_mobility ~ age + income + political_party + education +  urban_rural_class + federal_state, data = df1)
...
#### 3. Improving the regression fit


```{r}
## remove any 0 values in order to run a regression for Box-cox transformation 

df1_new <- df1 %>% filter(df1$CO2_mobility != 0)
nrow(df1_new)
```


```{r}
# Check the rows where the values are 0
# There are four data points where the CO2 mobility values are 0

df1 %>% filter_all(any_vars(. %in% c(0)))
...
```{r}
# Regression with all variables after dropping data point having 0 values in CO2 mobility  

model1_1 = lm(CO2_mobility  ~  age+ income + political_party + education +  urban_rural_class + federal_state, data = df1_new)
...
```{r}
# since the lambda value from the box-cox test is 0, y = log(y)

model1_2 = lm(log(CO2_mobility + 1)  ~  age + income + political_party + education +  urban_rural_class + federal_state  , data = df1)
...
max(10, 1/(1-summary(model1_2)$r.square))
...
influential = cooks.distance(model1_2)[which(cook >1)]
influential
...
#### 4. Assumptions check in the residuals of the transformed regression
...
res1 = stdres(model1_2) ## (Standardized) Residuals

# Linearity assumption/Mean zero assumption: still violated 
...
plot(fitted(model1_2), res1, xlab = "Fitted values", ylab = "Residuals")
abline(h = 0)
```
...
#### 5. Variable Selection, model outcome and assumption check 
...
step_model1 <- stepAIC(model1_2, trace=TRUE, direction= "backward")
summary(step_model1)
...
### III. Multivariate Regression: belief diff mobility
...
## defining a reference level: check chunk 11
...
model2 <- lm(belief_diff_mobility ~ age + income + political_party + education + urban_rural_class + federal_state, data = df2)
summary(model2) 
...
It does not work with this dataset because of the NaN 
...
```{r}
# After removing the outliers, R-squared improved
...
#plot(df2_scaled_new$urban_rural_class, res2, xlab = "urban_rural_class", ylab = "Residuals")
#abline(h = 0)

#plot(df2_scaled_new$education, res2, xlab = "education", ylab = "Residuals")
#abline(h = 0)

#plot(df2_scaled_new$federal_state, res2, xlab = "federal_state", ylab = "Residuals")
#abline(h = 0)

#plot(df2_scaled_new$political_party, res2, xlab = "Political Party", ylab = "Residuals")
#abline(h = 0)
