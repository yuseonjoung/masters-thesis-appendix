---
title: "Regression_Analysis_Total"
...
model1 <- lm(CO2_total ~ age + income + political_party + education +  urban_rural_class + federal_state, data = df1)
...
# Checking the outliers: estimate of the influence of data point; summary of how much a regression model changes when the ith observation is removed
...
Here is the final model for CO2 total and the selected independent variables: 
...
model1_trans <- lm(1/sqrt(CO2_total) ~ income + political_party, data = df1)
...
model2 <- lm(belief_diff_total ~ age + income + political_party + education +  urban_rural_class +federal_state , data = df2)
summary(model2) 
